...
from Correlation_utils import plot_3D
...
class NNCFModel(Model):
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            y_pred = self(initial_filter_matrix, training=True)[0]
    ...
    def test_step(self, data):
        _, ground_truth, sample_weights = data
        initial_filter_matrix = tf.zeros((1, 28, 28, 1))
        y_pred = self(initial_filter_matrix, training=False)[0]
...
def create_model(shape, num_correlations):
    ...
    model = NNCFModel(inputs, [outputs, cf])
...
print('wsh1:', train_data[0][0].shape)

nncf = create_model(num_correlations=num_correlations, shape=train_data[0][0].shape)
